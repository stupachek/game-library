...
func TestGamesSuccess(t *testing.T) {
	...
	t.Run("create, get game", func(t *testing.T) {
		...
		router.ServeHTTP(w, req)
		assert.Equal(t, 200, w.Code)
		assert.Regexp(t, "{\"data\":{\"gameId\":\"([a-zA-Z0-9-]{36})\",\"link\":\"https://game-library-docker.onrender.com/image/library/test.png\"},\"message\":\"Game is successfully created\"", w.Body.String())
